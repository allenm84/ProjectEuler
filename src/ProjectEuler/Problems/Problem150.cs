...
namespace ProjectEuler
{
  public class Problem150 : EulerProblem
  {
    ...
    private class Solution2 : EulerSolution
    {
      public override object GetSolution()
      {
        var s = new int[1024, 1024];
        var rs = new long[1024, 1024];
    ...
    private class Solution3 : EulerSolution
    {
      public override object GetSolution()
      {
        ...
        checked
        {
          ...
          var best = r[0]; // best value seen so far
          for (var i = 0; i < rows; ++i)
          {
            for (var j = 0; j <= i; ++j)
            {
              var k = i * (i + 1) / 2 + j;
              var b = s[k]; // temp best value starting at (i,j)
              best = Math.Min(best, b);
              for (var h = 1; h + i < rows; ++h)
              {
                b += r[(i + h) * (i + h + 1) / 2 + j + h];
                if (j > 0)
                {
